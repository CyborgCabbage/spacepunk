package cyborgcabbage.spacepunk.client.book;
...
public class MyPageMultiblock extends PageWithText {
    private static final Random RAND = Random.createLocal();
    ...
    @SerializedName("gui_offset") int guiOffset = 0;
    ...
    private void doWorldRenderPass(MatrixStack ms, AbstractMultiblock mb, Iterable<? extends BlockPos> blocks, final @NotNull VertexConsumerProvider.Immediate buffers, Vector4f eye) {
        for (BlockPos pos : blocks) {
            ...
            final BlockRenderManager blockRenderer = MinecraftClient.getInstance().getBlockRenderManager();
            if (!fluidState.isEmpty()) {
                final RenderLayer layer = RenderLayers.getFluidLayer(fluidState);
                final VertexConsumer buffer = buffers.getBuffer(layer);
                blockRenderer.renderFluid(pos, mb, new LiquidBlockVertexConsumer(buffer, ms, pos), bs, fluidState);
            }
