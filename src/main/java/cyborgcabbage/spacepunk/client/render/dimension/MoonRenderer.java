...
public class MoonRenderer implements DimensionRenderingRegistry.SkyRenderer {
    ...
    private VertexBuffer createAndFillBuffer(BufferMaker runnable){
    ...
    private BufferBuilder.BuiltBuffer renderSky(BufferBuilder builder, float coneHeight) {
    ...
    private BufferBuilder.BuiltBuffer renderStars(BufferBuilder buffer, long seed) {
    ...
    private void drawLightSky(WorldRenderContext context) {
        RenderSystem.setShader(GameRenderer::getPositionShader);
    ...
    private void drawBuffer(WorldRenderContext context, VertexBuffer buffer) {
        buffer.bind();
        buffer.draw(context.matrixStack().peek().getPositionMatrix(), context.projectionMatrix(), RenderSystem.getShader());
        VertexBuffer.unbind();
    }

    private void drawSunriseAndSunset(WorldRenderContext context) {
    ...
    private void drawDarkSky(WorldRenderContext context) {
        //Setup
        RenderSystem.setShader(GameRenderer::getPositionShader);
        RenderSystem.disableTexture();
    ...
    //Texture, Blend, Color, Shader Type
    private void drawBody(WorldRenderContext context, float angle, float distance, float size, Identifier texture, Vec2f uv1, Vec2f uv2){
        var bufferBuilder = Tessellator.getInstance().getBuffer();
        var matrices = context.matrixStack();
        var world = context.world();
        var tickDelta = context.tickDelta();
    ...
    private void drawBody(WorldRenderContext context, float angle, float distance, float size, Identifier texture){
        drawBody(context, angle, distance, size, texture, new Vec2f(0,0), new Vec2f(1,1));
    }

    private void drawPhasedBody(WorldRenderContext context, float angle, float distance, float size, Identifier texture, int textureWidth, int textureHeight, int phase){
    ...
    private void drawStars(WorldRenderContext context){
        RenderSystem.setShader(GameRenderer::getPositionShader);
        RenderSystem.disableTexture();
        if(starsBuffer == null)
